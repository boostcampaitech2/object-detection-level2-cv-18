...
ROOT = "./"
SUBMISSION_FILE_FORMAT = os.path.join("submissions_for_single_model", "submission*.csv")
ENSEMBLE_INFERENCE_CFG_FILE_NAME = "ensemble_inference_cfg.json"
ENSEMBLE_INFERENCE_CFG_FILE_PATH = os.path.join(ROOT, ENSEMBLE_INFERENCE_CFG_FILE_NAME)
SUBMISSION_CSV_PATH = os.path.join(ROOT, "submission.csv")


def get_submission_filenames():
    filenames = glob(os.path.join(ROOT, SUBMISSION_FILE_FORMAT))
    return filenames


def get_current_ensemble_cfg():
    with open(ENSEMBLE_INFERENCE_CFG_FILE_PATH, "r") as f:
        result = json.load(f)
    return result
    

def override_current_ensemble_cfg(cfg):
    with open(ENSEMBLE_INFERENCE_CFG_FILE_PATH, "w") as f:
        json.dump(cfg, f)
    

# 1. cfg.json 파일의 csv 항목을 
#    single model 의 submission file 이름으로 설정
#
# 2. 앙상블 결과로 나오는 csv 파일의 이름과 경로를 설정
def set_ensemble_cfg():
    cfg = get_current_ensemble_cfg()
    cfg['csv'] = get_submission_filenames()
    cfg['save_path'] = SUBMISSION_CSV_PATH
    override_current_ensemble_cfg(cfg)
...
def main():
    ...
    with open(os.path.join(ROOT, ENSEMBLE_INFERENCE_CFG_FILE_NAME), 'r') as f:
        cfgs = json.load(f)
