...
parser.add_argument('--csv', required=False, default='work_dirs/submission.csv')
parser.add_argument('--n', required=False, default='submission')
args = parser.parse_args()

root_dir = '/opt/ml/detection/dataset/'
save_path = f'/opt/ml/detection/mmdetection/submission_image/{args.n}/'
os.makedirs(save_path, exist_ok=True)

df = pd.read_csv(f'/opt/ml/detection/mmdetection/{args.csv}')
...
for i in tqdm(range(100)):    
    ...
    cv2.imwrite(os.path.join(save_path, image_id[i].replace('/', '_')), result_img)
